...
(defun test-validation-setup ()
  ...
  ;; Add test options
  (puthash "TEST_BOOL" '(:type bool :help "Test boolean option") linconf-kconfig-options)
  (puthash "TEST_TRISTATE" '(:type tristate :help "Test tristate option") linconf-kconfig-options)
  (puthash "TEST_STRING" '(:type string :help "Test string option") linconf-kconfig-options)
  (puthash "TEST_INT" '(:type int :range (0 . 100) :help "Test integer option") linconf-kconfig-options)
  (puthash "TEST_HEX" '(:type hex :range (#x10 . #xFF) :help "Test hex option") linconf-kconfig-options)
  (puthash "TEST_UNKNOWN" '(:type unknown :help "Test unknown type") linconf-kconfig-options))
...
(defun run-validation-tests ()
  ...
  (let ((test-results '())
    ...
    (let ((result (linconf-validate-option-value "TEST_BOOL" "y")))
      (push (list "bool y" (car result) (cdr result)) test-results))

    (let ((result (linconf-validate-option-value "TEST_BOOL" "n")))
      (push (list "bool n" (car result) (cdr result)) test-results))

    (let ((result (linconf-validate-option-value "TEST_BOOL" "m")))
