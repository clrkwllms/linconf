...
(defvar kconfig-vendor-specific-options
  '(;; RedHat/RHEL specific options
    ...
    ("WIRELESS_WDS" :type bool :deprecated t :version "6.10" :description "Wireless Distribution System - deprecated option")
    ("SLUB_MEMCG_SYSFS_ON" :type bool :deprecated t :version "6.12" :description "SLUB per-memcg sysfs cache - deprecated option")
    ("HARDENED_USERCOPY_PAGESPAN" :type bool :deprecated t :version "6.8" :description "Hardened usercopy pagespan checks - deprecated option")
    ("GCC_PLUGIN_CYC_COMPLEXITY" :type bool :deprecated t :version "6.5" :description "GCC cyclomatic complexity plugin - deprecated option")
    ("DEBUG_FORCE_FUNCTION_ALIGN_32B" :type bool :deprecated t :version "6.11" :description "Force 32-byte function alignment - deprecated debug option")
    ("X86_SMAP" :type bool :arch "x86" :description "Supervisor Mode Access Prevention support")
    ("USB_FTDI_ELAN" :type tristate :deprecated t :version "6.3" :description "USB FTDI ELAN adapter - deprecated driver")
    ("EFI_GENERIC_STUB_INITRD_CMDLINE_LOADER" :type bool :deprecated t :version "6.2" :description "EFI generic stub initrd cmdline loader - deprecated option")
    ("EEPROM_LEGACY" :type tristate :deprecated t :version "5.1" :description "I2C legacy EEPROM driver - deprecated")
    ("UNMAP_KERNEL_AT_EL0" :type bool :arch "arm64" :description "Unmap kernel when running in userspace (KPTI)")
    ("QCOM_QDF2400_ERRATUM_0065" :type bool :arch "arm64" :description "Qualcomm QDF2400 erratum 0065 workaround")
    ("QCOM_FALKOR_ERRATUM_1009" :type bool :arch "arm64" :description "Qualcomm Falkor erratum 1009 workaround")
    ("QCOM_FALKOR_ERRATUM_1003" :type bool :arch "arm64" :description "Qualcomm Falkor erratum 1003 workaround")
    ("SA_VIDEOCC_SA8775P" :type tristate :arch "arm64" :description "Qualcomm SA8775P automotive video clock controller")
    ("SA_DISPCC_SA8775P" :type tristate :arch "arm64" :description "Qualcomm SA8775P automotive display clock controller")
    ("SA_CAMCC_SA8775P" :type tristate :arch "arm64" :description "Qualcomm SA8775P automotive camera clock controller")
    ("PWM_RCAR" :type tristate :arch "arm64" :description "Renesas R-Car PWM controller")
    ("DRM_PANEL_SONY_TULIP_TRULY_NT3552" :type tristate :arch "arm64" :description "Sony Tulip Truly NT3552 panel driver")
    ("DRM_PANEL_SONY_ACX424AKP" :type tristate :arch "arm64" :description "Sony ACX424AKP panel driver")
